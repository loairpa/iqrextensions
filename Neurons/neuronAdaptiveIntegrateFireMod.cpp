...
/*
 * neuronAdaptiveIntegrateFireMod.cpp
 *
 * Based on: neuronAdaptiveIntegrateFire.cpp
 * Author: LIH
 */

#include "neuronAdaptiveIntegrateFireMod.h"
#include <time.h>
#include <valarray>

// Interface for dynamic loading is built using a macro.
MAKE_NEURON_DLL_INTERFACE(iqrcommon::ClsNeuronAdaptiveIntegrateFireMod,
			  "Adaptive integrate & fire Mod")

iqrcommon::ClsNeuronAdaptiveIntegrateFireMod::ClsNeuronAdaptiveIntegrateFireMod()
    : ClsNeuron(),
      pVmembrane(0),
      pActivity(0),
      pModulation(0)
{
    ...
    pModulation= addFeedbackState("modulation", "MODULATION");
    pModulationBuffer = addStateVariable("modTemp","Modulation from prev step");
 ...
 */
void iqrcommon::ClsNeuronAdaptiveIntegrateFireMod::update()
{
    ...
    StateArray &modulatory = getModulatoryInput();
    ...
    StateArray &modTemp    = pModulationBuffer ->getStateArray();
    ...
    // Modulation either 0 or 1
    for(unsigned int i=0; i<modulation[0].size();i++){
	modulation[0][i]=(abs(modulatory[0][i]+modTemp[0][i])>0.9)?1.0:0.0; // We only get modulatory input from 1 of the 12 neurons
       }

    // Store the modulatory input from before
    modTemp[0]=modulatory[0];
